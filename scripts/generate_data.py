...
def get_id():
    year = 2015 + random.randint(0, 4).zfill(4)
    college = random.randint(0, 15).zfill(2)
    major = random.randint(0, 40).zfill(2)
    sid = random.randint(0, 50).zfill(2)
    return year + college + major + sid


def generate_student(num):
    random.seed()
    for i in range(num):
        s = Student(name=get_name(), gender=('男', '女')[i % 2], student_id=get_id())
        s.save()
